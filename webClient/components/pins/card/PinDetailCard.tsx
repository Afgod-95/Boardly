...
const PinDetailCard = ({ pin, boards, onSave }: PinDetailCardProps) => {
  ...
  const mediaType = pin.video ? "video" : "image"
  const { handleCreateBoard } = usePinHook()
  ...
  return (
    <>
      <div className="relative border rounded-2xl w-full mx-auto space-y-5">
        ...
        {/* ── Sticky toolbar ── */}
        <div className="sticky top-15 bg-background rounded-t-2xl p-5 z-10 flex items-center gap-4 justify-between">
          <div className="flex items-center gap-3">
            ...
            <Popover>
              ...
              <PopoverContent side="bottom" align="start" className="w-sm rounded-2xl shadow-lg" sideOffset={8} forceMount>
                <SharePin pin={pin} onShare={() => toast.success(`Shared "${pin?.title}"`)} />
              </PopoverContent>
            </Popover>
            ...
            <Popover>
              ...
              <PopoverContent side="bottom" align="start" className="p-8 max-w-2xl rounded-2xl shadow-lg" sideOffset={8} forceMount>
          ...
          <div className="flex items-center gap-3">
            {pin?.isSaved !== true && pin?.boardId !== "profile" ? (
              <Link href={`/dashboard/boards/${pin?.boardId}`}
                className="flex items-center gap-2 py-2 px-4 hover:bg-accent rounded-full text-foreground transition-colors">
                <span className="text-base font-medium">
                  {boards.find(b => b.id === pin?.boardId)?.title}
                </span>
                <ChevronDown size={16} />
              </Link>
            ) : (
            ...
            <SaveButton pinId={pin?.id as string | number} 
              isSaved={pin?.isSaved as boolean} 
            />
          </div>
        </div>
        ...
        <div className="flex flex-col justify-center gap-4 px-5">
          <div className="relative max-w-xl mx-auto max-h-150 overflow-hidden rounded-xl">
            {mediaType === "image" ? (
              ...
              <video src={pin.video} controls className="w-full h-full object-contain" />
            )}
          </div>
              ...
              <div className="relative">
                ...
                >
                  <p className="text-base text-pretty leading-relaxed inline">
                    {pin.description}{" "}
                    {pin.description.length > MAX_TEXT && isExpanded && (
                  ...
                  {pin.description.length > MAX_TEXT && !isExpanded && (
                    <div className="absolute bottom-0 left-0 right-0 h-full bg-linear-to-r from-transparent to-background flex items-end justify-end pointer-events-none">
        ...
        {pin.author?.profileUrl ? (
          ...
          >
            ...
            <div>
              <span className="font-bold group-hover:underline block leading-tight">{pin.author?.name}</span>
        ...
        {/* ── Comments ── */}
        <CommentSection pinId={pin.id as string | number} />
      </div>

      {/* Outside the card/Popover — no focus conflicts */}
      <CreateBoardDialog
