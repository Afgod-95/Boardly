...
const SaveToBoard = ({
  pin, boards, onSave, onClose, onCreateBoardOpenChange,
}: SaveToBoardProps) => {

  const boardExists = boards.some((b) => b.id === pin.boardId)
  const initialBoard = pin.boardId && boardExists ? String(pin.boardId) : "profile"

  const [selectedBoard, setSelectedBoard] = useState<string>(initialBoard)
  const [saved, setSaved] = useState(false)

  const savedBoardName =
    selectedBoard === "profile"
      ? "your profile"
      : boards.find((b) => String(b.id) === selectedBoard)?.title ?? "board"
      ...
      <AnimatePresence mode="wait">
        ...
        {saved ? (
          ...
          >
            ...
            >
              <CheckCircle2 size={44} className="text-violet-500" strokeWidth={1.5} />
            </motion.div>
            <div>
              <p className="font-bold text-foreground text-base">Pin saved!</p>
              <p className="text-sm text-muted-foreground mt-0.5">
                Added to <span className="font-semibold text-foreground">{savedBoardName}</span>
              </p>
            </div>
          </motion.div>
            ...
            <div className="flex flex-col gap-6">
              <div className="flex flex-col gap-2">
                ...
                <div onClick={(e: React.MouseEvent) => e.stopPropagation()}>
                  <Select value={selectedBoard} onValueChange={(val) => setSelectedBoard(val)}>
                    <SelectTrigger className="w-full rounded-xl px-4 py-3 h-auto text-sm sm:text-base">
                      <SelectValue placeholder="Select a board" />
                    </SelectTrigger>
                    <SelectContent className="rounded-xl">
                      <SelectItem value="profile">Profile (no board)</SelectItem>
                      {boards.map((board) => (
